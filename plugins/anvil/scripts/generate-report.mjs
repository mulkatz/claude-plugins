#!/usr/bin/env bun
// Anvil Report Generator — Converts Markdown to a self-contained HTML report
...
:root {
  ...
  --advocate: #2d7a4f;
  --advocate-rgb: 45,122,79;
  --critic: #b83838;
  --critic-rgb: 184,56,56;
  --border: #d5d1ca;
  --border-light: #e8e4dd;
...
.toc-link.toc-advocate::before,
.toc-link.toc-critic::before {
  content: "";
...
.toc-link.toc-advocate::before { background: var(--advocate); }
.toc-link.toc-critic::before { background: var(--critic); }
...
.section > h2::before {
  counter-increment: anvil-section;
  content: counter(anvil-section, decimal-leading-zero);
...
.role-section h3 { margin: 0 0 0.75rem; }
...
.role-advocate {
  border-left-color: var(--advocate);
  background: linear-gradient(to right, rgba(var(--advocate-rgb),0.035), transparent 70%);
}

.role-badge.advocate {
  color: var(--advocate);
  background: rgba(var(--advocate-rgb),0.08);
}

.role-critic {
  border-left-color: var(--critic);
  background: linear-gradient(to right, rgba(var(--critic-rgb),0.035), transparent 70%);
}

.role-badge.critic {
  color: var(--critic);
  background: rgba(var(--critic-rgb),0.08);
}

.role-content > p:last-child { margin-bottom: 0; }
...
@media (max-width: 640px) {
  ...
  .role-section { padding: 0.75rem 1rem; }
...
@media print {
  ...
  pre, table, blockquote, .role-section, .executive-summary { page-break-inside: avoid; }
  .executive-summary { background: #faf7f0; border-left-color: #999; border-color: #ddd; }
  .role-section { background: none; border-left-color: #999; }
...
<div class="sidebar-brand"><span class="brand-mark"></span><span class="brand-text">ANVIL</span></div>
${buildTocHtml(tocItems)}
<div class="sidebar-footer">Generated by Anvil</div>
</nav>
...
<div class="report-eyebrow">ANVIL ANALYSIS</div>
